...
- [ ] 実機を接続し、開発者モードを有効化する
- [ ] 実機でアプリが起動できることを確認する
- [ ] 仕事場を1件以上登録し、対象仕事場の監視を有効化する（`monitoringEnabled = true`）
- [ ] 位置情報権限を `Always` に設定する
- [ ] 実施日・実施者・端末名・iOS バージョンを記録する
- [ ] `T-004` 手順1: 仕事場近傍から入域相当の移動を開始する
- [ ] `T-004` 手順2: 入域直後にアプリをタスクスイッチャーからキルする
- [ ] `T-004` 手順3: 5分以上の滞在条件を満たす
- [ ] `T-004` 手順4: アプリ再起動後に履歴と現在状態を確認する
- [ ] 勤務開始レコード、`entryTime`、`LocationProof`、異常ログ有無を確認する
- [ ] Pass/Fail 判定を記録する（欠落や整合性不一致があれば Fail）
- [ ] 実施ログを `docs/real_device_run_log_YYYY-MM-DD.md` に保存する
- [ ] `T-004` 結果を `simulator_test_plan.md` にも反映する
- [ ] 必要に応じて `T-001` `T-002` `T-006` `T-012` を実機で再確認する

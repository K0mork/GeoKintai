...
|---|---|---|---|---|---|---|
| FR-01 | 仕事場 CRUD | AC-01 | `WorkplaceRepositoryTests`, `PersistenceControllerTests`（`test_persistenceController_whenReinitializedWithSameStoreURL_restoresPersistedData`）, `AppStoreIntegrationTests`（`testAppStore_whenAddWorkplaceLatitudeOutOfRange_rejectsSave`, `testAppStore_whenAddWorkplaceLongitudeOutOfRange_rejectsSave`, `testAppStore_whenAddWorkplaceCoordinateHasWhitespace_savesSuccessfully`, `testAppStore_whenDeleteWorkplace_removesFromMonitoringSet`, `testAppStore_whenUpdateWorkplaceTargetMissing_setsError`, `testAppStore_whenUpdateWorkplaceLongitudeOutOfRange_rejectsUpdate`, `testAppStore_whenUpdateWorkplaceCoordinates_requestsRegionStateAgain`） | T-011 | P1-010, P1-020, P2-030, P2-031, P2-032 | Done |
...
| FR-04 | 退勤確定（再確認） | AC-04 | `ExitVerifierTests`, `AttendanceFlowIntegrationTests`, `AppStoreIntegrationTests`（`testAppStore_whenDidExitAndOutsideFor2Minutes_closesRecordAndSavesProof`, `testAppStore_whenDidExitButReturnInside_doesNotCloseAttendance`） | T-002, T-006 | P1-031, P3-020 | Done |
...
| FR-08 | エクスポート完全性 | AC-08 | `ExportServiceTests`（CSV/PDFのレコード完全性検証）, `AppStoreIntegrationTests`（`testAppStore_whenExportCSVNoData_showsReadableError`） | T-010 | P2-040, P2-041, P2-042, P3-030, P3-031, P3-032 | Done |
...
| FR-10 | 改ざん検知 | AC-10 | `IntegrityHashTests`, `ExportServiceTests`, `AppStoreIntegrationTests`（`testAppStore_whenTamperedCorrectionExists_marksAsIntegrityMismatch`） | T-010 | P1-050, P1-051, P1-060, P3-030 | Done |
